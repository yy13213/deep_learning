...
pip install -r exp/exp2/requirements.txt
```

### 运行原始 Baseline

```bash
python exp/exp2/rnn_lstm_sin.py \
  --epochs 300 \
  --lr 1e-2 \
  --hidden-size 20 \
  --num-layers 2 \
...
```bash
python exp/exp2/rnn_lstm_sin_optimized.py
```
...
```bash
python exp/exp2/rnn_lstm_sin_optimized.py --models gru
```
