...
class ResidualBlock(nn.Module):
    """
    基础残差块：Conv-BN-ReLU-Conv-BN，并通过 shortcut 建立残差连接。

    当输入输出通道数不同，或需要下采样时，shortcut 使用 1x1 卷积进行维度匹配；
    否则直接使用恒等映射。这样可以在加深网络的同时缓解梯度传播困难。
    """
...
class CNN(nn.Module):
    """
    残差增强版 CNN：输入 224x224。

    尺寸变化如下：
    stem: 224 -> 112
    residual stage 1: 112 -> 112
    residual stage 2: 112 -> 56
    residual stage 3: 56 -> 28
    residual stage 4: 28 -> 14
    residual stage 5: 14 -> 7
    ...
    def __init__(self, in_channels: int = 3, n_classes: int = 5, p_drop: float = 0.4):
        ...
        self.features = nn.Sequential(
            ResidualBlock(32, 32, stride=1, dropout2d=0.00),    # 112 -> 112
            ResidualBlock(32, 64, stride=2, dropout2d=0.00),    # 112 -> 56
            ResidualBlock(64, 64, stride=1, dropout2d=0.00),    # 56 -> 56
            ResidualBlock(64, 128, stride=2, dropout2d=0.05),   # 56 -> 28
            ResidualBlock(128, 128, stride=1, dropout2d=0.05),  # 28 -> 28
            ResidualBlock(128, 256, stride=2, dropout2d=0.10),  # 28 -> 14
            ResidualBlock(256, 256, stride=1, dropout2d=0.10),  # 14 -> 14
            ResidualBlock(256, 256, stride=2, dropout2d=0.10),  # 14 -> 7
        )
...
def run_smoke_test() -> None:
    ...
    loss = loss_fn(logits, y)
    loss.backward()
    print(f"[smoke] Residual-CNN: input={(4, 3, IMAGE_SIZE, IMAGE_SIZE)}, logits={tuple(logits.shape)}, loss={loss.item():.6f}")
...
def main() -> None:
    ...
    exp_dir = Path("experiment_outputs") / "cnn_flower_residual"
    ...
    for epoch in range(args.epochs):
        ...
        if va > best_val_acc:
            ...
            torch.save(
                ...
                out_model / "cnn_residual_best.pt",
            )
    ...
    torch.save(
        ...
        out_model / "cnn_residual_last.pt",
    )

    save_history_figure(history, exp_dir / "fig_residual_loss_acc.png")
    ...
    summary = {
        ...
        "saved_best_model": str((out_model / "cnn_residual_best.pt").resolve()),
        "saved_last_model": str((out_model / "cnn_residual_last.pt").resolve()),
    }
